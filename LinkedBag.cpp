...
LinkedBag<T> LinkedBag<T>::bagDifference(const LinkedBag<T>& a_bag) const
{
	...
	for(int i = 0; i < getCurrentSize(); i++)
	{
		if(contains(c_ptr->getItem()) && !a_bag.contains(c_ptr->getItem()))					// loops through the bag and checks if a_bag also has the
	...
	for(int j = 0; j < a_bag.getCurrentSize(); j++)
	{
		if(a_bag.contains(c_ptr->getItem()) && !contains(c_ptr->getItem()))					// loops through a_bag and checks if the bag also has the
